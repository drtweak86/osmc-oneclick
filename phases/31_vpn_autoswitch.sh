#!/usr/bin/env bash
# phases/31_vpn_autoswitch.sh (XBian-safe)
set -euo pipefail
IFS=$'\n\t'

# ---- Config (overrides via /etc/default/wg-autoswitch) ----------------------
...
# Adjust to your WireGuard profile basenames (without .conf)
...
# ---- Helpers / Toasts -------------------------------------------------------
# Must exist (your repo’s file provides toast_vpn_once, country_name_from_iface, quality_pct_from_speed)
. /opt/osmc-oneclick/phases/31_helpers.sh

# ---- Bail if critical binaries are missing ---------------------------------
need() { command -v "$1" >/dev/null 2>&1; }
for b in wg wg-quick ping curl; do
  if ! need "$b"; then
    echo "[oneclick][31_autoswitch] WARN: missing tool: $b" >&2
    exit 0
  fi
done
...
probe_throughput_mbit() {
  ...
  local bytes="$TEST_BYTES" start end dur_ms bits_per_s mbit
  start=$(date +%s%3N)
...
score_line() {
  local rtt_ms="$1" loss_pct="$2" dl_mbit="$3" loss_int
  loss_int=${loss_pct%.*}
  local loss_penalty=$(( loss_int * 50 ))
...
main() {
  ...
  declare -A rtt_map loss_map dl_map score_map
  ...
  for c in "${CANDIDATES[@]}"; do
    # Only consider candidates you actually have present
  ...
  if [[ -z "$best" ]]; then
    local best_dl=-1
    for c in "${!dl_map[@]}"; do
      d="${dl_map[$c]}"; (( d > best_dl )) && { best="$c"; best_dl="$d"; }
    done
  fi
